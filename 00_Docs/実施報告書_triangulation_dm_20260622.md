...
**作成日**: 2026年6月22日  
**プロジェクト**: NDB_XXX_triangulation_dm（NDB 方法論ライン・論文②）  
...
`projects/NDB_XXX_triangulation_dm/` 配下に以下の標準フォルダを作成した。
...
**ファイル**: `04_Manuscripts/Manuscript_triangulation_dm.qmd`（Quarto 形式）
...
| 優先度 | 作業 |
|--------|------|
| 高 | `quarto render Manuscript_triangulation_dm.qmd --to docx` で投稿用 DOCX 生成 |
