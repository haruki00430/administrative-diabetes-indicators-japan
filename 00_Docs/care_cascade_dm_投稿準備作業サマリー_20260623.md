...
S5感度分析・B001-27解釈段落の追加により、原稿の頑健性・解釈の明確さが向上したことを確認した（詳細は本ドキュメント4章）。複数の候補誌を比較検討した結果、最終的に **Annals of Epidemiology** に投稿した（2026-07-02、`04_Manuscripts/submission_package_Ann-Epi/` 参照）。

> 本セクションは2026-07-02、公開リポジトリ整備に伴い、候補誌ごとの受理確率推測（エージェントによる非公式な見立て）を削除し要旨のみに整理した。
